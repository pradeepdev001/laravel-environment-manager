...
<form method="GET" class="flex flex-wrap gap-3 mb-6">
    ...
    <select name="action" class="border border-gray-300 dark:border-gray-600 dark:bg-gray-800 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400">
        <option value="">All Actions</option>
        @foreach(['create','update','delete','rename','restore','bulk_update','reveal'] as $act)
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-750">
                <td class="px-4 py-2 text-xs text-gray-500 whitespace-nowrap">{{ $log->created_at->format('Y-m-d H:i:s') }}</td>
                <td class="px-4 py-2">
                    <span class="inline-block px-2 py-0.5 rounded-full text-xs font-medium
                        @if($log->action === 'reveal') bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300
                        @elseif($log->action === 'delete') bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300
                        @elseif($log->action === 'create') bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300
                        @else bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300
                        @endif">
                        {{ $log->action }}
                    </span>
                </td>
                <td class="px-4 py-2 font-mono text-xs text-gray-800 dark:text-gray-200">{{ $log->key }}</td>
                <td class="px-4 py-2 text-xs text-gray-600 dark:text-gray-400">{{ $log->user_name ?? '—' }}</td>
                <td class="px-4 py-2 text-xs text-gray-500">{{ $log->source }}</td>
                <td class="px-4 py-2 text-xs font-mono text-gray-500">{{ $log->ip_address ?? '—' }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="6" class="px-4 py-10 text-center text-gray-400">No audit records found.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
